...
var TypeInference = require("montage/core/meta/type-inference");
var infer = TypeInference.infer;
var engine = TypeInference.engine;
var Type = TypeInference.Type;
var types = TypeInference.types;

var TypeInferenceSpec = exports.TypeInferenceSpec = Montage.specialize({
}, {
...
describe("blueprints", function () {
    ...
    it("load a blueprint", function () {
        var inferSpec = new TypeInferenceSpec();
        var type = Type.of(inferSpec);
        expect(type.describe()).toEqual({
            type: "blueprint-promise"
        });

        return type.blueprintPromise.then(function () {
            var type = Type.of(inferSpec);
            expect(type.describe()).toEqual({
                type: "blueprint",
                name: "TypeInferenceSpec"
            });
        });
    });
    ...
    var scope;
    beforeEach(function () {
        var type = Type.of(new TypeInferenceSpec());
        scope = new Scope(type);
    });

    it("should infer type of a blueprint's property", function () {
        var expressionType = infer("number", scope);
        expect(expressionType.describe()).toEqual({type: "number"});
    });

    it("should infer type of a blueprint's collection property", function () {
        var expressionType = infer("strings", scope);
        expect(expressionType.describe()).toEqual({
            type: "array",
            contentType: {
                type: "string"
            }
    ...
    it("should infer type of expression from blueprint's collection property", function () {
        var expressionType = infer("strings.join(', ')", scope);
        expect(expressionType.describe()).toEqual({type: "string"});
    });
